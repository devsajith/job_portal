...
class Company {
    constructor(companyObj) {
        ...
        for (let job of companyObj.jobs) {
            if ((job.status === JobType.Published) && (this.jobs.length < 2)) {
...
class CompanyListView {
    constructor(companyDbObj, companyCount, hasNextPage) {
        this.companyList = [];
        for (let company of companyDbObj) {
            this.companyList.push(new Company(company));
        }
        this.totalCount = companyCount;
...
module.exports = CompanyListView;